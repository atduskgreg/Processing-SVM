...
class SVM {
  ...
  void loadModel(String filename) {
    try {
      model = svm.svm_load_model(dataPath(filename));
      svmProblem = new SVMProblem();
      svmProblem.numFeatures = model.nr_class;
    } 
    catch(IOException e) {
      println("ERROR loading model. Is the model file in the data folder? " + e);
    }
  }
  ...
  void saveModel(String filename) {
    try {
      svm.svm_save_model(dataPath(filename), model);
    } 
    catch(IOException e) {
      println("ERROR saving model. Does your sketch have a data folder? " + e);
    }
  }
